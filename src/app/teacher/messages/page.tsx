...
export default async function TeacherMessagesPage() {
  ...
  const [inbox, sent, recipients] = await Promise.all([
    ...
    prisma.user.findMany({
      where: { id: { not: user.id }, role: { in: ["ADMIN", "STUDENT"] } },
      orderBy: { name: "asc" },
    }),
  ]);
  ...
  const recipientOptions = recipients.map((u) => ({
    id: u.id,
    label: `${u.name} (${u.role === "ADMIN" ? "Admin" : "Student"})`,
  }));
  ...
  return (
    <div>
      <PageHeader
        title="Messages"
        description="Send and receive messages with the administration and students."
